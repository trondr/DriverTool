...
module Commands =
    ...
    type CommandDefinitions =        
        ...
        [<Command(Description = "Export remote update information for specified model to csv file. The update information is extracted from xml files located on the respective vendor web sites. Example for Lenovo: https://download.lenovo.com/catalog. Example for model 20EQ (Lenovo Thinkpad P50) and Win10: https://download.lenovo.com/catalog/20EQ_Win10.xml. The remote update information typically contain all available updates including those allready installed.", Summary = "Export remote update information for specified model to csv file.")>]
        ...
        [<Command(Description = "Export local update information for the current system to csv file. The current system is typically a reference machine and the exported info can be used to automate download of necessary updates to be installed on new systems with the same specification. It is required that vendor specific update utility (such as Lenovo System Update) is installed on the system and has been run to install all relevant updates. Currently only Lenovo System update is supported. Respective utilities for Dell and HP do not seem to support a 'tell me what you just installed' functionality.", Summary="Export local update information for the current system to csv file.")>]
        ...
        [<Command(Description="Create driver package for given manufacturer and model. Currently Lenovo is fully supported. Support for Dell and HP is in the works and supports creating driver package with all possible updates (needed and and unneeded alike) so for Dell and HP there is more requirement for manual decsion on what to include or not. If manufacturer or model or operating system is not given the manufacturer and model is automatically looked up.",Summary="Create driver package for given manufacturer and model.")>]
        static member CreateDriverPackage(
                                         ...
                                         [<OptionalCommandParameter(Description = "SystemFamily as specified by the SystemFamily property of the Win32_ComputerSystem wmi class instance. Powershell script to extract the system family: $(Get-WmiObject Win32_ComputerSystem|Select-Object SystemFamily).SystemFamily. If system family is not specified the current system family will be looked up and used.", ExampleValue = @"ThinkPad P50", AlternativeName = "sf", DefaultValue = "")>] 
                                            systemFamily : string,
                                         [<OptionalCommandParameter(Description = "Model code as specified by the Model property of the Win32_ComputerSystem wmi class instance. Powershell script to extract the model code for (1)LENOVO: $(Get-WmiObject Win32_ComputerSystem|Select-Object Model).Model.SubString(0,4); (2) Dell: $(Get-WmiObject Win32_ComputerSystem|Select-Object SystemSKUNumber).SystemSKUNumber; (3) HP: (Get-WmiObject -Class MS_SystemInformation -Namespace \"root\\WMI\"| Select-Object BaseBoardProduct).BaseBoardProduct. If model code is not specified, the current system model code will be looked up and used.", ExampleValue = @"20EQ", AlternativeName = "mo", DefaultValue = "")>] 
                                            modelCode : string,
